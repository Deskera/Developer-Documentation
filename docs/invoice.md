...
|salesInvoiceRecurring|`object`|Recurring details for the invoice. Attributes of [Sales Invoice Recurring Details](#sales-invoice-recurringdetails)|
|salesInvoiceItems        |`list`|List of items associated with invoice. Attributes of [Sales Invoice Items](#salesinvoiceitemdetails)|
|contact        |`object`|Contact information. Attributes of [ContactInfo](#contactinfo)|
...
---
### Retrieve Invoice
Retrieves the details of an existing invoice. You need only supply the unique invoice code that was returned upon invoice creation.
...
- limit `int`
- page `int`
- status `string` [Invoice Status](#invoice_status)
...
|type|`object`|Type of the item. Possible values [ProductType](#producttype)|
...
|recurrenceFrequency|`enum`|Recurrence frequency. Possible values [Recurring Frequency Type](#recurring_frequency_type)|
